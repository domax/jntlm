 ...
 *
 * <p>Ports CNTLM's {@code ntlm_hash_lm_password}, {@code ntlm_hash_nt_password} and
 * {@code ntlm2_hash_password} from {@code ntlm.c}. Each hash is a 16-byte value.
 */
public final class NtlmHashes {

    /** The fixed "KGS!@#$%" magic block DES-encrypted to derive the LM hash. */
    private static final byte[] LM_MAGIC =
    ...
    /**
     * Computes the 16-byte LM hash of a password. The password is upper-cased, truncated to
     * 14 bytes (OEM charset) and split into two 7-byte DES keys used to encrypt the magic block.
     */
    ...
    /**
     * Computes the 16-byte NTLMv2 hash (also called NTOWFv2): HMAC-MD5 keyed by the NT hash
     * over the UTF-16LE of upper-cased(username + domain).
     */
