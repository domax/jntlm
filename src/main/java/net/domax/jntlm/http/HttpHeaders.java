 ...
 *
 * <p>Mirrors the behaviour of CNTLM's {@code hlist_t}: header order is preserved, names are
 * matched case-insensitively, and duplicate header names are allowed. Provides the small set
 * of operations the proxy logic needs (add, replace/modify, delete, get, and token search).
 */
public final class HttpHeaders {
    ...
    /**
     * Adds or replaces a header (case-insensitive). If any header with this name exists, the
     * first occurrence is replaced and all further occurrences removed; otherwise it is appended.
     * Mirrors {@code hlist_mod(..., 1)}.
     */
    ...
    /**
     * Returns true if any header with the given name has a value that contains {@code token}
     * as a case-insensitive substring. Mirrors {@code hlist_subcmp}.
     */
