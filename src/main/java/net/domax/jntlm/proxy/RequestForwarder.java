 ...
 * goto bailout} jumps is reproduced with labelled loops and a {@code shortcut} flag.
 */
@Component
public class RequestForwarder {
  ...
  public RequestForwarder(
      ParentProxyManager parentProxyManager,
    ...
    while (true) {
      ...
      do {
        ...
        while (loop < 2) {
          ...
          if (loop == 1) {
            proxyAlive =
                data1.getHeaders().containsToken("Proxy-Connection", "keep-alive")
                    && data0.getHttpVersion() >= 11;
            if (!proxyAlive) {
  ...
  private void finishConnection(
      Endpoint server, Credentials tcreds, boolean proxyAlive, boolean authok) {
  ...
  private void trySend502(Endpoint client, HttpMessage request) {
    try {
      ErrorPages.send502(
          client.out(), request == null ? null : request.getProtocol(), "Bad Gateway");
